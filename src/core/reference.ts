import { ArmorCategory, EquipmentCategory, WeaponCategory, DamageType, XpLevel, Modifier, StatBlock, Skill, Alignment, ProficiencyLevel, Language, ToolProficiency, Backpack, Equipment } from "./types";
...
export class DndReference {
    ...
    public getLevel(xp: number): number {
        ...
        let lev = 0;
        for (let xpLevel of this.xpLevels) {
            if (xp >= xpLevel.xp)
                lev++;
            else
                break;
